...
主弹窗首次打开不使用横移或卡片上移动画，计数加载区保持固定行高；云端计数加载时显示「—」，不显示遮罩色块。
桌面主弹窗由容器绘制圆角，外层背景透明；手机端保持铺满。

「关于」页面可点击 MarkSync 访问本项目 GitHub 仓库，并注明复刻自 Yueby/bookmark-syncer，点击名称可访问原项目。
界面隐藏滚动条，仍可使用滚轮、触摸滑动和键盘滚动。
设置问号支持点击、悬停和键盘聚焦查看说明；保留份数仅显示一组加减按钮，备份文件间隔使用下拉选择。

新设备本地没有书签时，连接已有加密库后输入原密码开启加密；验证成功仅保存本机密码，再到同步面板拉取云端书签，无需发布清空。
...
| 📱 **极致双端适配** | 桌面端 360px 弹窗、宽屏限宽面板；移动端（Firefox Android / Kiwi）44px 列表操作热区、底部安全区与横竖屏适配；备份与快照面板固定高度、淡入显示，列表加载或展开时保持外框位置与宽度稳定 |
